@extends('bible::templates.frontend')
	  ...
	  @if (Auth::check())
		  ...
		  <div class="col-md-3">
			  ...
			  @if (isset($comment->commentable_type))
                @if ($comment->commentable_type=="Bishopm\Bible\Models\Blog")
                  (blog) - <a href="{{url('/')}}/blog/{{$comment->commentable->slug}}">
                @elseif ($comment->commentable_type=="Bishopm\Bible\Models\Sermon")
                  (sermon) - <a href="{{url('/')}}/sermons/{{$comment->commentable->series->slug}}/{{$comment->commentable->slug}}">
                @elseif ($comment->commentable_type=="Bishopm\Bible\Models\Course")
                  (course) - <a href="{{url('/')}}/course/{{$comment->commentable->slug}}">
                @elseif ($comment->commentable_type=="Bishopm\Bible\Models\Book")
...
@include('bible::shared.message-modal')
@endsection

@section('js')
<script type="text/javascript">
	@include('bible::shared.message-modal-script')
</script>
@stop